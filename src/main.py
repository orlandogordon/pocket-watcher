...
from parsers import amex, tdbank, amzn_syf, schwab, tdameritrade, ameriprise, fidelity, empower
...
def main():
    ...
    with open(credits_path, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows([['Date', 'Description', 'Amount', 'Bank Name', 'Account Holder', 'Account Number']])  # Consider adding 'Category', 'Tags', 'Account Nickname', 'Transaction Match' at the DB level
        print(f"Credits data CSV file created at: '{credits_path}'.")
    ...
    print('*'*100)
    print("Beginning 401k Statement Parsing Process")
    print('*'*100)

    for pdf_file in statements_path.glob('./fidelity/*.pdf'):
        parsed_data = fidelity.parse_401k_statement(pdf_file)
        fidelity.write_csv(brokerage_transactions_path, parsed_data.transaction_data)

    for pdf_file in statements_path.glob('./empower/*.pdf'):
        parsed_data = empower.parse_401k_statement(pdf_file)
        empower.write_csv(brokerage_transactions_path, parsed_data.transaction_data)
